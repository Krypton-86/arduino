...
/*0~9,
   ...
   23 r,
   24 c*/
int td = 10; //час між вимірюваннями для тиску в хвилинах
int timetoshift = minute();//технологічна змінна для вимірювання тиску через певний час(через td)
int timetosat = hour();//технологічна змінна для вимірювання для суми активних температур ЩОГОДИНИ
float SAT, SERperDAY;
float tempb1 = 10;
float tempb2 = 5.5;
int t1h, t2h, t3h, t4h, t5h, t6h, t7h, t8h, t9h, t10h, t11h, t12h, t13h, t14h, t15h, t16h, t17h, t18h, t19h, t20h, t21h, t22h, t23h, t24h;//змінні температури для САТ
...
void SAT() {
  ...
  int minn=minute();
  if (hourr == 1&&minn==30) {
    t1h = dht.readTemperature() * 10;
  }
  if (hourr == 2&&minn==30) {
    t2h = dht.readTemperature() * 10;
  }
  if (hourr == 3&&minn==30) {
    t3h = dht.readTemperature() * 10;
  }
  if (hourr == 4&&minn==30) {
    t4h = dht.readTemperature() * 10;
  }
  if (hourr == 5&&minn==30) {
    t5h = dht.readTemperature() * 10;
  }
  if (hourr == 6&&minn==30) {
    t6h = dht.readTemperature() * 10;
  }
  if (hourr == 7&&minn==30) {
    t7h = dht.readTemperature() * 10;
  }
  if (hourr == 8&&minn==30) {
    t8h = dht.readTemperature() * 10;
  }
  if (hourr == 9&&minn==30) {
    t9h = dht.readTemperature() * 10;
  }
  if (hourr == 10&&minn==30) {
    t10h = dht.readTemperature() * 10;
  }
  if (hourr == 11&&minn==30) {
    t11h = dht.readTemperature() * 10;
  }
  if (hourr == 12&&minn==30) {
    t12h = dht.readTemperature() * 10;
  }
  if (hourr == 13&&minn==30) {
    t13h = dht.readTemperature() * 10;
  }
  if (hourr == 14&&minn==30) {
    t14h = dht.readTemperature() * 10;
  }
  if (hourr == 15&&minn==30) {
    t15h = dht.readTemperature() * 10;
  }
  if (hourr == 16&&minn==30 {
    t16h = dht.readTemperature() * 10;
  }
  if (hourr == 17&&minn==30) {
    t17h = dht.readTemperature() * 10;
  }
  if (hourr == 18&&minn==30) {
    t18h = dht.readTemperature() * 10;
  }
  if (hourr == 19&&minn==30) {
    t19h = dht.readTemperature() * 10;
  }
  if (hourr == 20&&minn==30) {
    t20h = dht.readTemperature() * 10;
  }
  if (hourr == 21&&minn==30) {
    t21h = dht.readTemperature() * 10;
  }
  if (hourr == 22&&minn==30) {
    t22h = dht.readTemperature() * 10;
  }
  if (hourr == 23&&minn==30) {
    t23h = dht.readTemperature() * 10;
  }
  if (hourr == 0&&minn==30) {
    
    t24h = dht.readTemperature() * 10;
    SERperDAY = (t1h+t2h+t3h+t4h+t5h+t6h+t7h+t8h+t9h+t10h+t11h+t12h+t13h+t14h+t15h+t16h+t17h+t18h+t19h+t20h+t21h+t22h+t23h+t24h)/240;
    t1h=t2h=t3h=t4h=t5h=t6h=t7h=t8h=t9h=t10h=t11h=t12h=t13h=t14h=t15h=t16h=t17h=t18h=t19h=t20h=t21h=t22h=t23h=t24h=0;
    if (SERperDAY>tempb1){eeprom.write(1)=eeprom.read(1)+SERperDAY;}
    else if (SERperDAY>5.5){eeprom.write(2)=eeprom.read(2)+SERperDAY;}
    //виводити якщо замкнені контакти або кнопка
    //стерти кнопкою
    //розібратись з цілочисельними і дробовими даними (може в память не можна записувати дікі значення)
    //виводити почергово: |C 10| |eeprom.read(1)|*****|C 55| |eeprom.read(2)|
  }
}
